...
- Instead of following an efficiency model implemented by Matt Yang that doesn't respect the characteristics of each processor, change this dynamic and follow a more precise efficiency model, using the difference in the power of the LITTLE, BIG (and, if applicable, PRIME) cores, using the technologies these processors possess as a basis, allowing for a much better efficiency than what Matt Yang initially planned.
- Use PL1 (slowpowerlimit), PL2 (fastpowerlimit) and TEU(fastpowercapacity) that respects the device's TDP. Based on real data on costs, limits, base TDP, and actual power consumption for each load of each compatible SOC, this allows for wasteful power usage and improved heat dissipation by better recognizing the SOC's technical limitations. Overall, this, combined with our power model, allows Uperf to have a better power model than the less-than-well-optimized EAS models found in low-end devices.
...
- Follow the "rice-to-idle" strategy for interactions that need immediate demand, such as scroll, application launch and app switch. This approach prioritizes the maximum response capacity (RICE) during the brief interaction. The goal is to ensure that the task is completed in the shortest possible time, which in turn allows the system to return to the most efficient (idle) idle state faster. By minimizing the duration of high power use, this strategy significantly reduces aggregate energy consumption for the entire interaction cycle, effectively combating the waste of energy in peak loads facing the user.
...
- 支持Android 8 - 15
- 支持arm64-v8a
- Heavily modified kernels and ROMs may not support Uperf. More "stock" ROMs, such as AOSP and OEMs, are much more likely to be compatible, as these ROMs/kernels are somewhat "I don't like my users customizing my kernel/ROM."
